...
On-Care 는 이 세 가지 마찰을 정면으로 해결하기 위해, 사진 한 장으로 식단을 자동 분석하고 누적 이력을 RAG 로 참조해 *내 데이터를 아는* 맞춤 조언을 제공하는 것을 **핵심 설계 방향**으로 삼으며, 식단·운동·헬스장·일정·상담의 단일 앱 통합은 이미 MVP 로 구현했습니다.
...
> 💬 실제 사용자 인터뷰에서도 *"매번 식단을 일일이 검색하고 수동으로 입력하는 게 너무 번거로워 결국 중단하게 된다"* 는 의견이 지배적이었습니다.
...
├── backend/                    # 백엔드 (현재 Gemini Vision PoC · FastAPI 등 설계 예정)
│   ├── README.md               # 실행법 · 환경변수 · 로드맵
│   ├── requirements.txt
│   └── services/               # gemini_service.py (Vision AI 프로토타입) + 샘플 이미지
